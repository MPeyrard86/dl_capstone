...
if __name__ == '__main__':
    ...
    training_data = load_data([args.data_folder])

    images = np.asarray([x[0] for x in training_data], dtype=np.float32)
    images -= training_mean
    labels = np.asarray([x[2] for x in training_data], dtype=np.float32)

    # plt.imshow(images[0])

    train_time_end = time.time()
    print "Loaded %d samples in %.2fs." % (len(training_data), train_time_end - train_time_start)
    ...
    with tf.Session(graph=svhn_training_graph) as session:
        ...
        num_batches = int(math.ceil(len(images)/float(test_batch_size)))
        correct_predictions = 0
        total_predictions = 0
        for i in range(num_batches):
            lower_bound = i*test_batch_size
            upper_bound = (i+1)*test_batch_size
            upper_bound = upper_bound if upper_bound < len(images) else len(images)
            fd = {X: images[lower_bound:upper_bound]}
